...
class tables_hdlr:
    def __init__(self, imodel, log_level="DEBUG"):
        self.imodel = imodel
        self.logger = logging_hdlr.init_logger(__name__, level=log_level)
        self._imodel = f"{_base}.tables.{imodel}"
        try:
    ...
    def load_tables_maps(self, cdm_subset=None):
        map_paths = self._imodel_data.glob("*.json")
