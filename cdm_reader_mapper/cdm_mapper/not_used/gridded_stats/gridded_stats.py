...
# TO output nc
ENCODINGS = {
    "latitude": {"dtype": "int16", "scale_factor": 0.01, "_FillValue": -99999},
...
def from_cdm_monthly(
    ...
    qc_report_extensions = (
        "_".join(["qcr" + str(x) for x in qc_report]) if qc_report else None
    )
    ...
    canvas = create_canvas(
        REGIONS.get(region), DEGREE_FACTOR_RESOLUTION.get(resolution)
    )
    ...
    if not os.path.isfile(table_file):
        logging.error(f"Table file not found {table_file}")
        return
    df_header = pd.read_csv(
        table_file,
        delimiter=DELIMITER,
        usecols=READ_COLS_HDR,
    ...
    if qc_report:
        ...
        df_header = df_header.loc[
            df_header["report_quality"].isin([int(x) for x in qc_report])
        ]

    try:
        date_time = datetime.datetime(
            df_header["report_timestamp"][0].year,
            df_header["report_timestamp"][0].month,
            1,
        )
    except Exception:
        fields = cdm_id.split("-")
    ...
    for agg in AGGREGATIONS_HDR:
        xarr_dict[agg] = canvas.points(
            df_header, "longitude", "latitude", DS_AGGREGATIONS_HDR.get(agg)("crs")
        )
    ...
    for table in obs_tables:
        ...
        table_file = os.path.join(dir_data, "-".join([table, cdm_id]) + ".psv")
        if not os.path.isfile(table_file):
            logging.warning(f"Table file not found {table_file}")
            continue
        # Read the data
        df = pd.read_csv(
            table_file,
        ...
        try:
            date_time = datetime.datetime(
                df["date_time"][0].year, df["date_time"][0].month, 1
            )
        except Exception:
            fields = cdm_id.split("-")
            date_time = datetime.datetime(int(fields[0]), int(fields[1]), 1)
        ...
        nc_dir = dir_data if not nc_dir else nc_dir
        nc_name = (
            "-".join(filter(bool, [table, cdm_id, qc_report_extensions, qc_extensions]))
            + ".nc"
        )
        try:
...
def merge_from_monthly_nc(dir_data, cdm_id=None, nc_dir=None, force_header=True):
    ...
    if len(all_files) == 0:
        if force_header:
            logging.error(f"No nc files found {pattern}")
            return
        else:
            logging.warning(f"No header nc files found {pattern}")
    ...
    for table in obs_tables:
        ...
        all_files = glob.glob(os.path.join(dir_data, pattern))
        if len(all_files) == 0:
            logging.warning(f"No nc files found {pattern}")
            continue
        all_files.sort()
        # Read all files to a single dataset
        dataset = xr.open_mfdataset(all_files, concat_dim="time")
...
def global_from_monthly_cdm(
    ...
    if qc_report:
        logging.info("qc report extension")
    ...
    if len(all_files) == 0:
        logging.error(f"No files found {header_path}")
        return

    header_df = dd.read_csv(
        header_path,
        delimiter=DELIMITER,
        usecols=READ_COLS_HDR,
        parse_dates=["report_timestamp"],
    ...
    try:
        ...
        if not tables or "header" in tables:
            ...
            for agg in AGGREGATIONS_HDR:
                logging.info(f"Applying {agg} aggregation")
                xarr_dict[agg] = canvas.points(
                    header_df,
                    "longitude",
                    "latitude",
            ...
            nc_dir = dir_data if not nc_dir else nc_dir
            nc_name = (
                "-".join(filter(bool, [table, cdm_id, qc_report_extensions])) + ".nc"
            )
            xarr.to_netcdf(
                os.path.join(nc_dir, nc_name), encoding=ENCODINGS_HDR, mode="w"
            )
        else:
            shutil.rmtree(header_parq_path)
    except Exception:
    ...
    for table in obs_tables:
        ...
        if len(all_files) == 0:
            logging.warning(f"Table {table}. No files found {table_path}")
            continue
        ...
        try:
            ...
            nc_name = (
                "-".join(
                    filter(bool, [table, cdm_id, qc_report_extensions, qc_extensions])
                )
                + ".nc"
            )
            xarr.to_netcdf(
            ...
            logging.error(f"Error processing table {table}", exc_info=True)
            logging.info("Removing parquet files")
            shutil.rmtree(obs_parq_path)
            return
