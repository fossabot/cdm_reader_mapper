...
def read_tables(
    ...
    file_patterns = {
        tb: os.path.join(tb_path, "-".join([tb, tb_id]) + "." + extension)
        for tb in tables
    }
    file_paths = {}
    for k, v in file_patterns.items():
        logger.info(f"Getting file path for pattern {v}")
        file_path = glob.glob(v)
        if len(file_path) == 1:
            file_paths[k] = file_path[0]
        elif len(file_path) > 1:
            logger.error(
                f"Pattern {tb_id} resulted in multiple files for table {k}. "
                "Cannot seccurely retrieve cdm table(s)"
            )
            return

    if len(file_paths) == 0:
        logger.error(
            "No cdm table files found for search patterns {}: ".format(
                ",".join(list(file_patterns.values()))
            )
        )
        return
